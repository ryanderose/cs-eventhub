...
describe('default plan API', () => {
  ...
  it('seeds the default plan on first fetch', async () => {
    ...
    expect(payload.plan.blocks).toHaveLength(expectedSeedBlocks().length);
    const expectedOrder = expectedSeedBlocks().map((_, index) => index);
    expect(payload.plan.blocks.map((block) => block.order)).toEqual(expectedOrder);
  ...
  it('updates block ordering and enforces optimistic concurrency', async () => {
    ...
    const expectedOrder = expectedSeedBlocks().map((_, index) => index);
    expect(updated.plan.blocks.map((block) => block.order)).toEqual(expectedOrder);
